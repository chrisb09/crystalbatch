...
def read_words():
    ...
    text_file = open(
        os.path.join(dirname, "res/txt/words.txt"),
        "r",
        encoding="utf-8",
        errors="ignore",
    )
...
def update_top_bar_filename(path):
    ...
    line_amount_req = 1
    print(file_name)
    while line_amount_req * chars_per_line * line_inc_char_rate ** (
        line_amount_req - 1
    ) < len(file_name):
        line_amount_req += 1
    fs = int(init_fs / line_amount_req)
    label.config(font=("Courier", fs))
    file_name_adjusted = split_text_after_max(
        file_name, int(chars_per_line * line_inc_char_rate ** (line_amount_req - 1))
    )
...
class GifThread(Thread):
    ...
    def run(self):
        ...
        while self.stop == False:
            ...
            factor = min(
                limit[0] / current[0],
                (limit[1] - (TITLE_HEIGHT + LOWER_BAR_HEIGHT)) / current[1],
            )
            self.image_object.seek(self.index % self.image_object.n_frames)
            logo = ImageTk.PhotoImage(
                self.image_object.resize(
                    (int(current[0] * factor), int(current[1] * factor)),
                    Image.ANTIALIAS,
                )
            )
            now = time.time()
            ...
            self.next_frame_timestamp = (
                now + self.image_object.info["duration"] / 1000.0
            )
            if self.stop == False:
...
def set_image(path):
    ...
    if file_extension.lower() in IMAGE_FILE_EXTENSIONS:
        ...
        logo = ImageTk.PhotoImage(
            image_f.resize(
                (int(current[0] * factor), int(current[1] * factor)), Image.ANTIALIAS
            )
        )
        image.config(image=logo)
        image.image = logo
        ...
        center_window.place(
            ...
            y=TITLE_HEIGHT,
            width=limit[0],
            height=limit[1] - (TITLE_HEIGHT + LOWER_BAR_HEIGHT),
        )
        image.place_forget()
        image.place(
            x=(limit[0] - int(current[0] * factor)) // 2,
            y=0
            + (
                (
                    (limit[1] - (TITLE_HEIGHT + LOWER_BAR_HEIGHT))
                    - int(current[1] * factor)
                )
                // 2
            ),
            width=int(current[0] * factor),
            height=int(current[1] * factor),
        )
        if (
            gif_thread is None
            or gif_thread.is_alive() == False
            or gif_thread.gif_path is None
            or not gif_thread.gif_path == path
        ):
...
for i in range(len(sources)):
    ...
    for (dirpath, _, filenames) in os.walk(source):
        for filename in filenames:
            ...
            if (
                file_extension.lower() in IMAGE_FILE_EXTENSIONS
                or file_extension.lower() in VIDEO_FILE_EXTENSIONS
            ):
...
files.sort(
    key=lambda x: int.from_bytes(
        hashlib.md5(bytes(os.path.basename(x[0]), "utf-8")).digest(), "big"
    )
)
...
for f in files:
    ...
    print(
        str(
            int.from_bytes(
                hashlib.md5(bytes(os.path.basename(f[0]), "utf-8")).digest(), "big"
            )
        )
        + " '"
...
def configure_event(event):
    ...
    if limit[0] != window.winfo_width() or limit[1] != window.winfo_height():
        ...
        label.place_forget()
        label.place(
            x=TITLE_HEIGHT, y=0, width=limit[0] - 2 * TITLE_HEIGHT, height=TITLE_HEIGHT
        )
        button_f11.place_forget()
        button_f11.place(
            x=limit[0] - TITLE_HEIGHT, y=0, width=TITLE_HEIGHT, height=TITLE_HEIGHT
        )
        ...
        button_enter.place(
            x=limit[0] - LOWER_BAR_HEIGHT,
            y=0,
            width=LOWER_BAR_HEIGHT,
            height=LOWER_BAR_HEIGHT,
        )
        framebottom.place_forget()
        framebottom.place(
            x=0, y=limit[1] - LOWER_BAR_HEIGHT, width=limit[0], height=LOWER_BAR_HEIGHT
        )
...
escape_b = ImageTk.PhotoImage(
    Image.open("res/img/escape.png").resize(
        (TITLE_HEIGHT, TITLE_HEIGHT), Image.ANTIALIAS
    )
)
...
f11_b = ImageTk.PhotoImage(
    Image.open("res/img/f11.png").resize((TITLE_HEIGHT, TITLE_HEIGHT), Image.ANTIALIAS)
)
button_f11 = Button(
    window, text="F11!", width=TITLE_HEIGHT, height=TITLE_HEIGHT, fg="red", image=f11_b
)
...
label = tk.Label(text="Empty...", fg="red", bg="white", width=100, height=2)
...
delete_b = ImageTk.PhotoImage(
    Image.open("res/img/delete.png").resize((100, 100), Image.ANTIALIAS)
)
button_delete = Button(
    framebottom, text="Delete!", width=100, height=100, fg="red", image=delete_b
)
button_delete.bind("<Button-1>", delete)
enter_b = ImageTk.PhotoImage(
    Image.open("res/img/enter.png").resize((100, 100), Image.ANTIALIAS)
)
button_enter = Button(
    framebottom, text="Enter!", width=100, height=100, fg="red", image=enter_b
)
...
button_enter.place(
    x=limit[0] - LOWER_BAR_HEIGHT, y=0, width=LOWER_BAR_HEIGHT, height=LOWER_BAR_HEIGHT
)
framebottom.place(
    x=0, y=limit[1] - LOWER_BAR_HEIGHT, width=limit[0], height=LOWER_BAR_HEIGHT
)
